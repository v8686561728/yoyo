...
//         cardExpiryDateValue: DateFormatter(this.currentGiftCard.cardExpiryDate),
...
//       this.updatePayload = {...this.updatePayload, cardExpiryDate: cardExpiryDate};
//     } else {
//       this.setState({
//         cardExpiryDateValue: value,
...
//     let value = event.target.value;
//     if (/(http(s?):)([/|.|\w|\s|-])*\.(?:jpg|gif|png)/.test(value) && value.length > 0 && value.length < 10000) {
//       this.setState({
...
//               this.props.adminUpdateCard(this.currentGiftCard.id, this.updatePayload).then((response) => {
//                 this.setState({
...
//           <Button variant="contained" style={{marginLeft:'20px'}} onClick={this.clearInput}>CLEAR</Button>
//           <Button variant="contained" style={{marginLeft:'10px'}} color="primary" onClick={this.addUpdateCard}>{(this.props.match.params.id) ? 'UPDATE' : 'ADD'}</Button>
//         </div>
//           <div>
// {this.state.showErrorSnackBar ? 
//   <Snackbar message={"Validation failed or some fields are missing"} color="red" />
//  : null}
